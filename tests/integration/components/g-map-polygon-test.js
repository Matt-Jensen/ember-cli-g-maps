...
import {makeTrianglePath} from 'ember-cli-g-maps/components/g-map-polygon/component';

moduleForComponent('g-map-polygon', 'Integration | Component | g map polygon', {
  integration: true
});

test('it renders polygon on a g-map instance', function(assert) {
  this.render(hbs`{{#g-map as |map|}}{{g-map-polygon map}}{{/g-map}}`);

  return getGoogleMapPolygons(this.$('.ember-cli-g-map')).then((polygons) => {
    assert.equal(polygons.length, 1, 'rendered a single g-map-polygon on g-map instance');
  });
});

test('it renders a triangular polygon in center of map if a path is not given', function(assert) {
...
test('it renders all polygon path strategies', function(assert) {
  ...
  this.render(hbs`{{#g-map as |map|}}
    {{g-map-polygon map path=path}}
    {{g-map-polygon map options=options}}
  {{/g-map}}`);
...
test('it sets clickable', function(assert) {
  ...
  this.render(hbs`{{#g-map as |map|}}
    {{g-map-polygon map clickable=clickable}}
    {{g-map-polygon map options=options}}
  {{/g-map}}`);
...
test('it sets draggable', function(assert) {
  ...
  this.render(hbs`{{#g-map as |map|}}
    {{g-map-polygon map draggable=draggable}}
    {{g-map-polygon map options=options}}
  {{/g-map}}`);
...
test('it sets editable', function(assert) {
  ...
  this.render(hbs`{{#g-map as |map|}}
    {{g-map-polygon map editable=editable}}
    {{g-map-polygon map options=options}}
  {{/g-map}}`);
...
test('it sets fillColor', function(assert) {
  ...
  this.render(hbs`{{#g-map as |map|}}
    {{g-map-polygon map fillColor=fillColor}}
    {{g-map-polygon map options=options}}
  {{/g-map}}`);
...
test('it sets fillOpacity', function(assert) {
  ...
  this.render(hbs`{{#g-map as |map|}}
    {{g-map-polygon map fillOpacity=fillOpacity}}
    {{g-map-polygon map options=options}}
  {{/g-map}}`);
...
test('it sets geodesic', function(assert) {
  ...
  this.render(hbs`{{#g-map as |map|}}
    {{g-map-polygon map geodesic=geodesic}}
    {{g-map-polygon map options=options}}
  {{/g-map}}`);
...
test('it sets strokeColor', function(assert) {
  ...
  this.render(hbs`{{#g-map as |map|}}
    {{g-map-polygon map strokeColor=strokeColor}}
    {{g-map-polygon map options=options}}
  {{/g-map}}`);
...
test('it sets strokeOpacity', function(assert) {
  ...
  this.render(hbs`{{#g-map as |map|}}
    {{g-map-polygon map strokeOpacity=strokeOpacity}}
    {{g-map-polygon map options=options}}
  {{/g-map}}`);
...
test('it sets strokePosition', function(assert) {
  ...
  this.render(hbs`{{#g-map as |map|}}
    {{g-map-polygon map strokePosition=strokePositionName}}
    {{g-map-polygon map options=options}}
  {{/g-map}}`);
...
test('it sets strokeWeight', function(assert) {
  ...
  this.render(hbs`{{#g-map as |map|}}
    {{g-map-polygon map strokeWeight=strokeWeight}}
    {{g-map-polygon map options=options}}
  {{/g-map}}`);
...
test('it sets visible', function(assert) {
  ...
  this.render(hbs`{{#g-map as |map|}}
    {{g-map-polygon map visible=visible}}
    {{g-map-polygon map options=options}}
  {{/g-map}}`);
...
test('it sets zIndex', function(assert) {
  ...
  this.render(hbs`{{#g-map as |map|}}
    {{g-map-polygon map zIndex=zIndex}}
    {{g-map-polygon map options=options}}
  {{/g-map}}`);
...
test('it consumes `options` hash along with top-level values', function(assert) {
  ...
  this.render(hbs`{{#g-map as |map|}}
    {{g-map-polygon map options=options zIndex=zIndex}}
  {{/g-map}}`);

  return getGoogleMapPolygons(this.$('.ember-cli-g-map')).then(([polygon]) => {
    assert.equal(polygon.zIndex, zIndex, 'set zIndex via top-level property');
    assert.equal(polygon.clickable, clickable, 'set clickable via options');
  });
});
...
test('it provides the default mouse event argument to all click actions', function(assert) { //   assert.expect(10);
  ...
  return getGoogleMapPolygons(this.$('.ember-cli-g-map')).then(([polygon]) => {
    ...
    // Trigger all mouse events on polygon
    mouseEvents.forEach((event) =>
      google.maps.event.trigger(polygon, event, stubMouseEvent));
  });
});
 ...
 * @return {Array}   Google Map Polygons array
 */
function getGoogleMapPolygons(element) {
